...
def pad_cube_to_shape(cube, target_shape=(60, 60)):
    """
    Pads each 2D slice of a 3D cube to the given target_shape (centered padding).

    Parameters:
        cube: 3D numpy array of shape (n_slices, height, width)
        target_shape: tuple (target_height, target_width) for each slice

    Returns:
        3D numpy array of shape (n_slices, *target_shape)
    """
    padded_slices = []
    ...
    for slice_ in cube:
        ...
        padded = np.pad(slice_,
                        ((pad_y // 2, pad_y - pad_y // 2),
                         (pad_x // 2, pad_x - pad_x // 2)),
                        mode='constant')
        padded_slices.append(padded)

    return np.array(padded_slices)


def match_cube_to_shape(cube, target_shape):
    """
    Adjust only spatial dimensions (axis 1 and 2) to target_shape.
    Axis 0 (spectral) is preserved exactly.
    """
