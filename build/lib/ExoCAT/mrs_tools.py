...
def load_cube_dir_ch(path, idx = 1, suffixe = 's3d'):
    files = [f for f in os.listdir(path) if f.split("_")[-1]==f"{suffixe}.fits"]
    file_id = [f.split("_")[idx] for f in files]
    # print(file_id)
    band_cubes = {"ch1-short": "1A", "ch1-medium": "1B", "ch1-long": "1C",
                  ...
                  "ch3-short": "3A", "ch3-medium": "3B", "ch3-long": "3C",
                  "ch4-short": "4A", "ch4-medium": "4B", "ch4-long": "4C",
                  "ch1-shortmediumlong": "1", "ch2-shortmediumlong": "2", "ch3-shortmediumlong": "3",
                  "ch4-shortmediumlong": "4"}
...
# For STPSF precomputed PSFs
def load_psfs(fits_filename):
    from astropy.io import fits
    """Charge les PSFs et les longueurs d’onde depuis un fichier FITS."""
    with fits.open(fits_filename) as hdul:
        psfs = hdul[0].data
        wavelengths = hdul['WAVELENGTHS'].data
    return psfs, wavelengths
