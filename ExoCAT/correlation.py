...
def load_spectrum(path, Rp=1, dstar=1,
                  path_wave="/Users/mmalin/Models_spectra/ExoREM_2026/wavenumber.h5",
                  degraded=False, res=None, plot=False, savefig=False):
    """
    Processes an ExoREM spectrum (version 2026) for MIRI-MRS.
    ...
    Returns:
        tuple: 
            - Wavelength array (µm).
            - Flux array (µJy).
            - If degraded=True, returns the degraded spectrum (wavelength, flux).
    """

    # Planet and star parameters
    R = Rp * 69911 * 1e5          # Jupiter radius -> cm
    d = dstar * 3.26 * 9.46e17    # parsec -> cm

    # Load ExoREM spectral flux
    try:
        with h5py.File(path, "r") as hf:
            fk = hf["flux"][:]  # flux [W/m²/cm⁻¹]
    except Exception as e:
        raise RuntimeError(f"Error reading HDF5 file {path}: {e}")

    # Dilution by planet size and distance
    flux_dil = fk * (R**2 / d**2)  # scaled flux [W/m²/cm⁻¹]

    # Load wavenumber
    with h5py.File(path_wave, "r") as kf:
        wn = kf["wavenumber"][:]  # wavenumber [cm⁻¹]

    # Convert wavenumber to wavelength (µm)
    wavelength = 1e4 / wn  # cm^-1 -> µm

    # Convert flux from W/m²/cm⁻¹ to µJy
    flux_conv = flux_dil * (1 / 3e10) * 1e26 * 1e6
...
def load_spectrum_old(path, Rp=1.0, dstar=1.0, degraded=False, res=None,
                  plot=False, savefig=False):
    """
    Load and process an ExoREM spectrum (grid 2021 version).
    Parameters
    ----------
    path : str
        Path to the ExoREM spectrum text file.
    Rp : float
        Planet radius in Jupiter radii.
    dstar : float
        Distance to the star in parsecs.
    degraded : bool
        If True, degrade the spectrum to resolution 'res'.
    res : float
        Spectral resolution (required if degraded=True).
    plot : bool
        If True, plot the spectrum.
    savefig : bool
        If True, save the plot as PNG.

    Returns
    -------
    tuple (wavelength [µm], flux [µJy])
        If degraded=True → returns degraded spectrum.
    """
    data = np.loadtxt(path)
    wavenumber = data[:, 0]        # cm⁻¹
    spectral_flux = data[:, 1]     # W.m⁻².cm⁻¹

    R_JUP_KM = 69911.0
    PC_TO_KM = 3.26 * 9.46e12

    Rp_km = Rp * R_JUP_KM
    d_km = dstar * PC_TO_KM

    flux_diluted = spectral_flux * (Rp_km**2 / d_km**2)
    # Wavenumber (cm⁻¹) → wavelength (µm)
    wavelength = 1e4 / wavenumber

    # Flux: W.m⁻².cm⁻¹ → µJy
    c_cgs = 3e10  # cm/s
    flux = flux_diluted * (1 / c_cgs) * 1e26 * 1e6

    # Reverse to increasing wavelength
    wavelength = np.flip(wavelength)
    flux = np.flip(flux)
    if degraded:
        if res is None:
            raise ValueError("res must be provided if degraded=True")
        wavelength_deg, flux_deg = degrade_spectrum(wavelength, flux, res)
    if plot:
        plt.figure(figsize=(14, 4))
        plt.plot(wavelength, flux, label="ExoREM model")

        if degraded:
            plt.plot(wavelength_deg, flux_deg,
                     label=f"Degraded (R={res})")

        plt.xlabel("Wavelength (µm)", fontsize=14)
        plt.ylabel("Flux (µJy)", fontsize=14)
        plt.legend()
        plt.xlim(0, 30)

        if savefig:
            plt.savefig("spectre_planet.png", dpi=300)

        plt.show()
    if degraded:
        return wavelength_deg, flux_deg
    else:
...
def get_exorem_spectra(ds, teff_array, logg, met, co, fsed, RJup, dist_star):
    """
    Extract and flux-calibrate ExoREM spectra for several Teff values at once
    from an already-open grid dataset.

    Parameters
    ----------
    ds : xarray.Dataset
        Open ExoREM grid dataset (xr.open_dataset(..., engine="netcdf4")).
    teff_array : array-like
        Effective temperatures to extract [K] (par1).
    logg, met, co, fsed : float
        Fixed grid parameters (par2, par3, par4, par5).
    RJup : float
    ...
    -------
    wave : ndarray
        Wavelength grid [µm], shared across all Teff.
    flux_dict : dict[float, ndarray]
        Flux-calibrated spectrum [µJy] for each requested Teff.
    """
    wave = ds['grid']['wavelength'].values  # µm

    # One vectorized nearest-neighbour selection across all Teff at once,
    # instead of one .sel() call per temperature.
    grid_sel = ds['grid'].sel(
        par1=teff_array, par2=logg, par3=met, par4=co, par5=fsed,
        method="nearest",
    ).transpose("par1", "wavelength").values  # shape (n_teff, n_wave), W/m²/um

    # Planet and star parameters, converted to cm
    R_cm = RJup * 69911 * 1e5           # Jupiter radii -> cm
    d_cm = dist_star * 3.26 * 9.46e17   # pc -> cm
    dilution = R_cm**2 / d_cm**2

    lam_m = wave * 1e-6                 # µm -> m
    c_m_s = 3e8

    flux_dict = {}
    for teff, flux in zip(teff_array, grid_sel):
        flux_dil = flux * dilution              # W/m²/um
        flux_si = flux_dil * 1e6                 # W/m²/m
        flux_nu = flux_si * lam_m**2 / c_m_s      # W/m²/Hz
        flux_dict[teff] = flux_nu * 1e26 * 1e6     # µJy

    return wave, flux_dict